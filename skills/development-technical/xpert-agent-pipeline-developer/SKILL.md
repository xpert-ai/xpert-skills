...
- Apply its [Primary Agent Middleware Defaults](../xpert-assistant-dsl-builder/references/primary-agent-middleware-defaults.md) to the Orchestrator and every independent role Assistant's primary Agent. Reuse one baseline generator; private internal specialists receive only their own required capabilities. Todos and compressed summaries do not determine DAG completion or replace persisted node outputs.
...
- [case-project-assistant-workspace-binding.md](references/case-project-assistant-workspace-binding.md) when each persisted Case must own an Xpert Project used as the shared file workspace for its Orchestrator and role Assistant Tasks, including plugin-side use of Project provisioning, Managed Queue, Assistant Task, Workbench navigation, failure recovery, and non-compatible plugin-data reset.
