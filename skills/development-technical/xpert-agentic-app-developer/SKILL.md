...
Before designing a new Workbench or Extension View, or substantially changing its information architecture or visual hierarchy, read [references/enterprise-shadcn-design-principles.md](references/enterprise-shadcn-design-principles.md) and treat it as the product-design standard. For every React Remote View that uses the shared shadcn package or Tailwind semantic color utilities, also read [references/shadcn-ui.md](references/shadcn-ui.md) before implementation. Treat host theme installation as a runtime contract: load the shared stylesheet, map host `--xui-*` tokens to shadcn semantic variables through the shared installer, propagate density, keep the documented CSS fallback, and verify computed styles in the installed host iframe.
