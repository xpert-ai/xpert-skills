...
- Compose interactive controls from `@xpert-ai/plugin-shadcn-ui`; do not substitute native selects, ad hoc buttons, copied Radix source, emoji, or CSS-drawn control glyphs when a shared primitive or standard icon exists.
- Compile Tailwind CSS from the maintained TSX source. Loading a prebuilt shared stylesheet does not compile consumer utility classes; configure the Remote View build to scan its own `src/**/*.{ts,tsx}` files and emit the production `app.css`.
- Load the shared shadcn stylesheet once, apply host `--xui-*` tokens, then call the repository's canonical semantic-variable installer (`installShadcnThemeVars`; `installShadcnCssVar` is only a compatibility alias where a package exposes it) after initialization and on theme changes. Do not treat stylesheet loading as theme installation.
